...
export function ActionToolbar({
  ...
  return (
    <div
      className={cn(
        "relative z-0 flex w-fit flex-wrap items-center rounded-xl border border-muted bg-gradient-to-b from-background to-muted/30 p-1 shadow-sm",
        className,
      )}
    >
      {buttons.map((btn, index) => {
        ...
        const buttonClasses = cn(
          "flex items-center gap-2 px-3 h-9 rounded-md transition-all duration-200",
          isActive
            ...
            <div key={index} className="flex items-center">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    variant="ghost"
                    className="ml-0.5 h-9 w-8 rounded-md hover:bg-muted/80"
                  >
