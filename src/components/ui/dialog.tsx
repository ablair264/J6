...
type DialogActionIntent = "plain" | "outline" | "primary" | "danger"
...
function mapDialogIntentToVariant(
  intent: DialogActionIntent,
...
const DialogClose = ({ intent = "plain", ref, ...props }: DialogButtonProps) => {
  return (
    <Button
      slot="close"
      ref={ref}
      variant={mapDialogIntentToVariant(intent)}
      {...props}
    />
  )
}

interface CloseButtonIndicatorProps extends Omit<DialogButtonProps, "children"> {
  className?: string
...
const DialogCloseIcon = ({ className, ...props }: CloseButtonIndicatorProps) => {
  return props.isDismissable ? (
    <PrimitiveButton
      ...
      slot="close"
      className={cx(
        "close absolute end-1 top-1 z-50 grid size-8 place-content-center rounded-md hover:bg-secondary focus-visible:bg-secondary focus-visible:outline-hidden focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] sm:end-2 sm:top-2 sm:size-7",
        className,
      )}
    >
